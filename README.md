...
Use the following minimal layout (no package):

```text
CalculatorApp/
├─ README.md               # This file (bilingual guide)
├─ Operation.java           # abstract base class
├─ BinaryOperation.java     # abstract two-operand helper
├─ UnaryOperation.java      # abstract one-operand helper
├─ Addition.java            # implements binary addition
├─ Division.java            # implements binary division with error message constant
├─ Power.java               # implements binary power operation
├─ SquareRoot.java          # implements unary square root with negative input constant
└─ CalculatorApp.java       # main app with Scanner and menu
```

Notes / ملاحظات:
- Keep classes small and focused to clearly demonstrate OOP principles.
- CalculatorApp should have a parameterized constructor that accepts a `Scanner` for automated testing.
- Keep shared error messages as public static final constants placed in the classes that use them.

## Common commands / أوامر شائعة
- Build: `javac *.java`
- Run: `java CalculatorApp`

## Testing tips / نصائح للاختبار
- Use the parameterized `CalculatorApp(Scanner)` to inject a `Scanner` over a `String` for unit tests.
- Test edge cases: division by zero, negative input for square root, and non-numeric input handling.
...
- **العلاقات**:
  - الوراثة: BinaryOperation ← Operation، و UnaryOperation ← Operation، و Addition/Division/Power ← BinaryOperation، و SquareRoot ← UnaryOperation.
  - الارتباط/التركيب: الفئة CalculatorApp تمتلك مرجعًا من نوع `Operation` (`currentOperation`) للتفويض.
  - الواجهات: غير مستخدمة التزامًا بالمواصفات التي طلبت فئات مجردة فقط؛ تم تحقيق التجريد عبر الفئات المجردة.
- **مخطط UML**: المخطط النصي في التقرير يذكر الأسماء والحقول والطرائق وعلاقات الوراثة/التركيب، وهو مطابق للتنفيذ.
- **التجريد**: الفئات المجردة `Operation`, `BinaryOperation`, `UnaryOperation` تقدم عقود `calculate` مجردة؛ لم تُستخدم واجهات وفقًا للتصميم المطلوب.

### متطلبات البرمجة (مطابقة لهذا المشروع)
- **عدد الأصناف**: 8 أصناف (المواصفة الأصلية فرضت 8 بالضبط، أي أقل من شرط “10 أو أكثر” العام).
- **الوراثة**: مطبقة عبر الهيكل المجرد والفئات الفرعية الملموسة.
- **التركيب**: الفئة CalculatorApp تملك كائن `Operation` للتنفيذ.
- **التغليف (الدوال الوصول/التعديل)**: المتغير `symbol` محمي مع Getter؛ باقي الحالة خاصة (`scanner`, `currentOperation`).
- **البُناة**: بُناة افتراضية ومُعَلَّمة (مثال: CalculatorApp مع/بدون Scanner؛ العمليات تقبل رموزًا مخصصة).
- **التعددية الشكلية**: إعادة تعريف `calculate` في كل عملية ملموسة؛ الاستدعاء يتم عبر مراجع `Operation`.
- **التجريد**: فئات مجردة كما ذُكر؛ لم تُضَف واجهات التزامًا بعدد الأصناف المحدد.
- **الثوابت**: `Division.ERROR_MESSAGE` و `SquareRoot.NEGATIVE_INPUT_MESSAGE` معرفتان كثوابت `public static final`.
- **الفئة الرئيسة**: `CalculatorApp` تحتوي `main` ومسار الاختبار.
- **التفاعل مع المستخدم**: استخدام `Scanner` لاختيارات القائمة والمدخلات العددية.
- **إخراج واضح**: قوائم وأوامر واضحة، تنسيق نتيجة، ورسائل خطأ ودية للمدخلات غير الصالحة أو الحالات الرياضية الخاطئة.
- **تطابق التصميم مع التنفيذ**: هيكل الشيفرة يطابق مخطط UML ووصف المسؤوليات في التقرير.
